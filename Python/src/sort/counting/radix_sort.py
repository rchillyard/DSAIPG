...
NOTE unlike every other sort in this tree, ``to`` here is INCLUSIVE, following
the Java. Note also that only non-negative values work: a negative number gives
a negative digit and so a negative bucket.
"""
...
def find_max_int(num_arr: list[int], from_: int, to: int) -> int:
    ...
    :param to: the last index to consider, INCLUSIVE.
    :return: the largest value in the range.
    """
    return max(num_arr[from_:to + 1])
...
def count_sort(num_arr: list[int], exp: int, from_: int, to: int) -> None:
    ...
    :param to: the last index to sort, INCLUSIVE.
    """
    ...
    for i in range(to, from_ - 1, -1):
        digit = (num_arr[i] // exp) % RADIX
        count[digit] -= 1
        result[count[digit] + from_] = num_arr[i]
    if to + 1 - from_ >= 0:
        num_arr[from_:to + 1] = result[from_:to + 1]
...
def sort(num_arr: list[int], from_: int, to: int) -> None:
    ...
    if to > len(num_arr) - 1:
        raise IndexError(f"To should be between 0 and {len(num_arr) - 1}")
