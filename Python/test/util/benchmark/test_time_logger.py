...
class TestFormatTime:
    ...
    def test_it_matches_the_java(self, value, expected):
        # Checked against DecimalFormat with the pattern "######.0000". Note the
        # missing leading zero, and note that the integer part is never
        # truncated to the six "#" of the pattern.
        assert format_time(value) == expected

    def test_a_small_negative_number_keeps_its_sign(self):
        assert format_time(-0.5) == "-.5000"
...
class TestTimeLogger:
    def test_a_raw_time_is_logged_unchanged(self, caplog):
        with caplog.at_level(logging.INFO, logger="src.util.benchmark.time_logger"):
            TimeLogger("Raw time per run (mSec):", None).log("MergeSort", 1.5, 1000)
        assert "MergeSort: Raw time per run (mSec): 1.5000" in caplog.text
