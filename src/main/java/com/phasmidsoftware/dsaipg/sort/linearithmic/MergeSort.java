 ...
 */
 ...
 public class MergeSort<X extends Comparable<X>> extends SortWithComparableHelper<X> {
     ...
     private InsertionSort<X> setupInsertionSort(final Helper<X> helper) {
         return new InsertionSort<>(helper);
     }
     ...
     public void sort(X[] a, int from, int to) {
         ...
         if (noCopy) {
             sort(aux, a, from, to); // aux: input, a: output
         } else {
     ...
     private void sort(X[] a, X[] aux, int from, int to) {
        ...
        boolean noCopy = config.getBoolean(MERGESORT, NOCOPY);if (to <= from + helper.cutoff()) { // XXX check that a cutoff value of 1 effectively stops the cutoff mechanism.
