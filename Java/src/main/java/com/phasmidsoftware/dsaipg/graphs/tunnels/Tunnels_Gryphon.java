...
import com.phasmidsoftware.gryphon.java.MinimumSpanningTree;
import com.phasmidsoftware.gryphon.java.WeightedEdge;
import com.phasmidsoftware.gryphon.java.WeightedGraph;
...
public class Tunnels_Gryphon implements Iterable<WeightedEdge<Building, TunnelProperties>> {

    public static void main(String[] args) throws IOException {
        Tunnels_Gryphon ts = new Tunnels_Gryphon(BuildingLoader.createBuildings());
        Collection<WeightedEdge<Building, TunnelProperties>> mst = ts.getMst();

        double totalCost = mst.stream()
                .map(WeightedEdge::attribute)
                .mapToDouble(tp -> tp.cost)
                .sum();
        System.out.println("Total cost: " + totalCost);

        System.out.println("Number of edges: " + mst.size());

        Graph_Edges<Building, TunnelProperties> graphEdges = new Graph_Edges<>();
        for (WeightedEdge<Building, TunnelProperties> e : mst) {
            Edge<Building, TunnelProperties> edge = new Edge<>(e.from(), e.to(), e.attribute());
            graphEdges.addEdge(edge);
        }
        Kml<Building, TunnelProperties> kml = new Kml<>(graphEdges);
...
//        System.out.println("Total cost: " + totalCost + ", total length: " + totalLength);
    ...
    public Tunnels_Gryphon(List<Building> buildings) {
        WeightedGraph<Building, TunnelProperties> g = WeightedGraph.undirectedWeighted();
        setupZones();
        setupTunnels();
        EdgeGraph<Building, TunnelProperties> tunnelGraph = createGraph(buildings, e -> e.getAttribute().length <= 250);
        SizedIterable<Edge<Building, TunnelProperties>> edges = tunnelGraph.edges();
        for (Edge<Building, TunnelProperties> e : edges) {
            Building destinationBuilding = e.getOther(e.get());
            g.addEdge(new WeightedEdge<>(e.get(), destinationBuilding, e.getAttribute()));
        }
        mst = MinimumSpanningTree.prim(g, buildings.get(5), Comparator.comparingLong(tp -> tp.cost));
    }
    ...
    @NotNull
    @Override
    public Iterator<WeightedEdge<Building, TunnelProperties>> iterator() {
        return mst.values().iterator();
    }

    public Collection<WeightedEdge<Building, TunnelProperties>> getMst() {
        return mst.values();
    }

    public Map<Building, WeightedEdge<Building, TunnelProperties>> getMstMap() {
        return mst;
    }
    ...
    final private Map<Building, WeightedEdge<Building, TunnelProperties>> mst;
}